...
export class BravoLayoutData {
    ...
    public static async createNew(xml: any, userOptions?: any): Promise<BravoLayoutData> {
        if (xml instanceof Array) {
            try {
                let _zLayout: string = null;
                await BravoZipTool.open(xml).then(async data => {
                    _zLayout = await data.readEntry('Xml');
                });
        ...
        if (wjc.isString(xml))
            return this.createNewString(xml, options);
    }
    ...
    public static createNewString(xml: string, options?: any): BravoLayoutData {
        ...
        if (_l instanceof BravoLayoutData) {
            if (_l.contains(RootName))
                return _l.getChildLayoutData(RootName);
            else if (_l.count == 1)
    ...
    public toString() {
        let _parentNode = document.createElement(RootName);
        _parentNode.setAttribute(TagName, RootName);
